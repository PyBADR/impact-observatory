...
export async function POST(req: NextRequest, { params }: Params) {
  ...
  if (backend) {
    try {
      const res = await fetch(`${backend}/api/v1/decisions/${decision_id}/close`, {
        method: "POST",
  ...
  const dec = serverStore.decisions.close(decision_id);
  if (!dec) {
    ...
    return NextResponse.json({ error: "Decision already closed" }, { status: 409 });
  }
